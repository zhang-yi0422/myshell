...
#include<string.h>
#include<time.h>//获取时间
#include<sys/stat.h>//返回一个包含文件属性的结构体
#include<unistd.h>//调用系统原语
#include<sys/types.h>//一些数据类型，如pid_t进程ID
#include<linux/limits.h>//限制数据类型边界
#include<dirent.h>//打开目录读取目录关闭目录，成功则返回指针
#include<grp.h>//组文件
#include<pwd.h>//口令文件
...
void display(int flag, char *pathname)//根据参数决定如何显示单个文件
{
    int i,j;
    struct stat buf;
